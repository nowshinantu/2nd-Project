        ...
        <div class="row">
            <div class="col-5">
                <form action="" class="login-form"  method="post">
                    ...
                    <input type="text" class="form-control mb-3" name="administration">

                    <!-- <label for="">Short Details</label>
                    <input type="text" class="form-control mb-3" name="deptshort"> -->
    ...
    $res = getDataFromDB($sql);
                    foreach($res as $row){
                        ...
                        <tr>
                            ...
                            <td><!--<a href="dptwiseemployee.php?id=<?php echo $row['serialno'] ?>" class="btn btn-warning btn-sm">View</a>
                                    <a href="editdept.php?id=<?php echo $row['serialno'] ?>" class="btn btn-warning btn-sm">Edit</a>  -->
                                    <a href="deletedept.php?<?php echo $row['serialno'] ?>" class="btn btn-danger btn-sm">Delete</a></td>
                        </tr>
                        <?php
                    }
                    ?>
...
{
    include_once('../db/dbconnect.php');
    $sql = "SELECT * FROM administration ORDER BY SerialNo DESC Limit 1";
    $res = getDataFromDB($sql);
    $count = count($res);
    if($count == 0){
        $id = date('Y').'-'.'1';
    ...
    $insert = "INSERT INTO `administration`(`admin_name`) VALUES ('".$_POST["administration"]."')";
