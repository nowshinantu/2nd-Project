...
<div class="modal" id="myModal">
  <div class="modal-dialog">
    ...
    $res = getDataFromDB($sql);
                    foreach($res as $row){
                        ?>
                        <option value="<?php echo $row['OPositionID'] ?>"><?php echo $row['OPositionID'].' - '.$row['Position'] ?></option>
        ...
        <div class="row">
            <div class="col-12">
                <table class="table table-bordered table-striped">
                    ...
                    <?php
                    include_once('../db/dbconnect.php');
    $sql = "SELECT * FROM employees inner join administration on administration.SerialNo = employees.Dept_ID inner join office_position on office_position.OPositionID = employees.Pos_ID WHERE UserRole = 'OfficeStaff'";
    $res = getDataFromDB($sql);
                    foreach($res as $row){
                        ?>
                        <tr>
                            ...
                            <!-- <a href="editdept.php?<?php echo $row['Dept_ID'] ?>" class="btn btn-outline-warning btn-sm">Edit</a> -->
                             <a href="delete/deletedept.php?id=<?php echo $row['Dept_ID'] ?>" class="btn btn-outline-danger btn-sm">Delete</a></td>
                        </tr>
...
{
    include_once('../db/dbconnect.php');
    $sql = "SELECT * FROM office ORDER BY SerialNo DESC Limit 1";
    $res = getDataFromDB($sql);
    $count = count($res);
    if($count == 0){
        $id = date('Y').'-1-0-'.'1';
...
$insert = "INSERT INTO `employees`( `Emp_ID`, `FName`, `LName`, `Email`, `Password`, `UserRole`, `Dept_ID`, `Pos_ID`, `Grade`) VALUES ('".$id."','".$_POST['FName']."','".$_POST['LName']."','".$_POST['Email']."','".md5(12345)."','OfficeStaff','".$_POST['Dept_ID']."','".$_POST['Position_ID']."','".$_POST['Grade']."')";
